import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import { render, screen, cleanup, fireEvent, waitFor } from '@testing-library/react';

// ---------------------------------------------------------------------------
// PowerPage already has PowerPage.test.tsx covering live stats, range
// switching, offset navigation, data states, and CSV export. This file adds
// coverage for the untested logic: spike removal in the chart data, the
// PDF/HTML consumption report export (window.open path), the cost overlay
// from /api/report, and the cost fallback config hydration from /api/settings.
// ---------------------------------------------------------------------------

type ApiGetCall = { path: string };

const apiGetCalls: ApiGetCall[] = [];
const apiGetMock = vi.fn(async (path: string) => {
  apiGetCalls.push({ path });
  if (path.startsWith('/api/report')) {
    return {
      ok: true,
      import_cost_gbp: 1.50,
      export_income_gbp: 0.30,
      net_cost_gbp: 1.20,
      standing_charge_gbp: 0.50,
      standing_charge_p_per_day: 0.50,
      days_in_range: 1,
    };
  }
  if (path === '/api/settings') {
    return {
      ok: true,
      data: {
        import_tariff: 0.285,
        export_tariff: 0.15,
        import_tariff_config: null,
        export_tariff_config: null,
        import_standing_charge_p_per_day: 50,
      },
    };
  }
  return { ok: true, data: {} };
});

const fetchHistoryMock = vi.fn();

vi.mock('../../src/lib/api', () => ({
  apiGet: (...args: unknown[]) => apiGetMock(...(args as [string])),
  fetchHistory: (...args: unknown[]) => fetchHistoryMock(...args),
  getApiBase: () => 'http://localhost:7337',
  getServerPort: () => 7337,
  isTauri: false,
}));

vi.mock('recharts', () => ({
  ResponsiveContainer: ({ children }: { children: React.ReactNode }) => <div>{children}</div>,
  ComposedChart: ({ children }: { children: React.ReactNode }) => <div>{children}</div>,
  Area: () => null,
  Line: () => null,
  ReferenceLine: () => null,
  CartesianGrid: () => null,
  Tooltip: () => null,
  XAxis: () => null,
  YAxis: () => null,
}));

globalThis.ResizeObserver = class {
  observe() {}
  unobserve() {}
  disconnect() {}
};

import PowerPage from '../../src/pages/PowerPage';
import { useInverterStore } from '../../src/store/useInverterStore';
import type { InverterSnapshot } from '../../src/lib/types';

function silenceConsoleError() {
  return vi.spyOn(console, 'error').mockImplementation(() => {});
}

function makeSnapshot(overrides: Partial<InverterSnapshot> = {}): InverterSnapshot {
  return {
    timestamp: Math.floor(Date.now() / 1000),
    solar_power: 3000, pv1_power: 2000, pv2_power: 1000,
    pv1_voltage: 0, pv2_voltage: 0, pv1_current: 0, pv2_current: 0,
    battery_power: -500, soc: 60, battery_voltage: 50, battery_current: 0,
    battery_state: 'charging', battery_temperature: 20, battery_capacity_kwh: 9.5,
    eps_power_w: 0, grid_power: 200, grid_voltage: 240, grid_frequency: 50,
    grid_online: true, grid_loss: false, inverter_trip: false,
    battery_over_temp: false, home_power: 800, inverter_temperature: 30,
    inverter_time: '',
    today_solar_kwh: 0, today_pv1_kwh: 0, today_pv2_kwh: 0,
    today_import_kwh: 0, today_export_kwh: 0, today_charge_kwh: 0,
    total_import_kwh: 0, total_export_kwh: 0, total_solar_kwh: 0,
    total_charge_kwh: 0, total_discharge_kwh: 0, total_throughput_kwh: 0,
    operating_hours: 0, today_discharge_kwh: 0, today_consumption_kwh: 0,
    home_energy_today_kwh: 0, battery_modules: [], battery_mode: 'eco',
    battery_reserve: 4, charge_rate: 0, discharge_rate: 0, active_power_rate: 0,
    max_battery_power_w: 0, max_ac_power_w: 0, export_limit_w: 0, target_soc: 100,
    enable_charge_target: false, enable_charge: false, enable_discharge: false,
    auto_winter_active: false, load_limiter_active: false, cosy_active: false,
    cosy_enabled: false, agile_active: false, agile_state: 'idle', agile_enabled: false,
    max_charge_slots: 0, max_discharge_slots: 0, charge_slots: [], discharge_slots: [],
    meters: [], inverter_serial: '', firmware_version: '', dsp_firmware_version: '',
    dc_dsp_firmware_version: '', device_type: '', device_type_display: 'Gen 3 Hybrid',
    device_type_code: '2201', battery_calibration_stage: 0, enable_ammeter: false,
    enable_reversed_ct_clamp: false, meter_type: 0, supports_battery_calibration: false,
    ac_eps_enabled: false, ac_export_priority: 0,
    ...overrides,
  };
}

/** Generate a series with a single spike point to exercise removePowerSpikes. */
function seriesWithSpike(): Record<string, { t: number; v: number }[]> {
  const base = 1000;
  const t0 = 1_700_000_000_000;
  const points = [];
  for (let i = 0; i < 5; i++) {
    points.push({ t: t0 + i * 30_000, v: base });
  }
  // Insert a spike in the middle (index 2): 5000W with neighbors at 1000W.
  points[2] = { t: t0 + 2 * 30_000, v: 5000 };
  return {
    solar_power: points,
    battery_power: points.map((p) => ({ ...p, v: -p.v })),
    grid_power: points,
    home_power: points,
    soc: points.map((p) => ({ ...p, v: 50 })),
  };
}

describe('<PowerPage/> — spike removal, cost overlay, PDF export', () => {
  beforeEach(() => {
    silenceConsoleError();
    apiGetCalls.length = 0;
    apiGetMock.mockClear();
    fetchHistoryMock.mockClear();
    useInverterStore.setState({
      snapshot: null,
      chartRange: '24h',
      gridLineWeight: 'standard',
    });
  });

  afterEach(() => {
    vi.restoreAllMocks();
    cleanup();
  });

  describe('cost overlay from /api/report', () => {
    it('fetches /api/report on mount with the current range', async () => {
      fetchHistoryMock.mockResolvedValue({});
      render(<PowerPage />);
      await waitFor(() => {
        const reportCalls = apiGetCalls.filter((c) => c.path.startsWith('/api/report'));
        expect(reportCalls.length).toBeGreaterThan(0);
        expect(reportCalls[0]!.path).toContain('range=24h');
      });
    });

    it('refetches /api/report when paging back with offset=1', async () => {
      fetchHistoryMock.mockResolvedValue({});
      render(<PowerPage />);
      await waitFor(() => {
        expect(apiGetCalls.some((c) => c.path.startsWith('/api/report'))).toBe(true);
      });
      // Page back.
      fireEvent.click(await screen.findByRole('button', { name: /Older/i }));
      await waitFor(() => {
        const offsetCalls = apiGetCalls.filter((c) => c.path.includes('offset=1'));
        expect(offsetCalls.length).toBeGreaterThan(0);
      });
    });
  });

  describe('cost fallback config from /api/settings', () => {
    it('hydrates the cost fallback from settings on mount', async () => {
      fetchHistoryMock.mockResolvedValue({});
      render(<PowerPage />);
      await waitFor(() => {
        expect(apiGetCalls.some((c) => c.path === '/api/settings')).toBe(true);
      });
    });
  });

  describe('Consumption Report export', () => {
    it('opens a new window for the PDF/HTML report in browser mode (not Tauri)', async () => {
      fetchHistoryMock.mockResolvedValue(seriesWithSpike());
      const mockWindow = {
        document: {
          open: vi.fn(),
          write: vi.fn(),
          close: vi.fn(),
        },
        focus: vi.fn(),
      };
      const openSpy = vi.spyOn(window, 'open').mockReturnValue(mockWindow as never);
      render(<PowerPage />);
      // Wait for data to load (hasData becomes true).
      await waitFor(() => {
        const csvBtn = screen.getByRole('button', { name: 'CSV' });
        expect(csvBtn.hasAttribute('disabled')).toBe(false);
      });
      fireEvent.click(screen.getByRole('button', { name: /Consumption Report/i }));
      await waitFor(() => {
        expect(openSpy).toHaveBeenCalledTimes(1);
      });
      expect(mockWindow.document.write).toHaveBeenCalled();
      expect(mockWindow.document.close).toHaveBeenCalled();
      // Toast confirms it "opened in a new window".
      await waitFor(() => {
        expect(screen.getByText(/opened in a new window/)).toBeDefined();
      });
    });

    it('downloads the report as a file when window.open returns null (popup blocked)', async () => {
      fetchHistoryMock.mockResolvedValue(seriesWithSpike());
      vi.spyOn(window, 'open').mockReturnValue(null);
      const createObjectUrlMock = vi.fn(() => 'blob:report');
      const revokeObjectUrlMock = vi.fn();
      const anchorClickMock = vi.fn();
      Object.defineProperty(URL, 'createObjectURL', { configurable: true, value: createObjectUrlMock });
      Object.defineProperty(URL, 'revokeObjectURL', { configurable: true, value: revokeObjectUrlMock });
      vi.spyOn(HTMLAnchorElement.prototype, 'click').mockImplementation(anchorClickMock);

      render(<PowerPage />);
      await waitFor(() => {
        expect(screen.getByRole('button', { name: 'CSV' }).hasAttribute('disabled')).toBe(false);
      });
      fireEvent.click(screen.getByRole('button', { name: /Consumption Report/i }));
      await waitFor(() => {
        expect(createObjectUrlMock).toHaveBeenCalled();
      });
      // Toast says "downloaded".
      await waitFor(() => {
        expect(screen.getByText(/downloaded to your Downloads folder/)).toBeDefined();
      });
    });

    it('flashes a CSV export toast with the period label', async () => {
      fetchHistoryMock.mockResolvedValue(seriesWithSpike());
      vi.spyOn(window, 'open').mockReturnValue(null);
      const createObjectUrlMock = vi.fn(() => 'blob:csv');
      const revokeObjectUrlMock = vi.fn();
      const anchorClickMock = vi.fn();
      Object.defineProperty(URL, 'createObjectURL', { configurable: true, value: createObjectUrlMock });
      Object.defineProperty(URL, 'revokeObjectURL', { configurable: true, value: revokeObjectUrlMock });
      vi.spyOn(HTMLAnchorElement.prototype, 'click').mockImplementation(anchorClickMock);

      render(<PowerPage />);
      await waitFor(() => {
        expect(screen.getByRole('button', { name: 'CSV' }).hasAttribute('disabled')).toBe(false);
      });
      fireEvent.click(screen.getByRole('button', { name: 'CSV' }));
      await waitFor(() => {
        expect(screen.getByText(/CSV downloaded to your Downloads folder/)).toBeDefined();
      });
    });
  });

  describe('snapshot-driven derived values', () => {
    it('shows the snapshot timestamp in the header when connected', async () => {
      const ts = 1_700_000_000;
      useInverterStore.setState({ snapshot: makeSnapshot({ timestamp: ts }) });
      render(<PowerPage />);
      // The header shows the local time of the snapshot.
      expect(screen.getByText(new Date(ts * 1000).toLocaleTimeString())).toBeDefined();
    });

    it('shows "Waiting for data" in the header when no snapshot', async () => {
      render(<PowerPage />);
      // "Waiting for data" also appears in the stat tiles, so there are
      // multiple matches — the header is the right-aligned one.
      expect(screen.getAllByText('Waiting for data').length).toBeGreaterThan(0);
    });
  });
});
