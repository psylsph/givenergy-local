...
class PrepareMsixTests(unittest.TestCase):
    def test_msix_version_accepts_release_semver(self) -> None:
        self.assertEqual(prepare_msix.msix_version("v0.66.3"), "1.66.3.0")
        self.assertEqual(prepare_msix.msix_version("1.2.3-beta.1"), "2.2.3.0")

    def test_msix_version_rejects_invalid_or_oversized_components(self) -> None:
        for version in ("1.2", "1.two.3", "1.2.3.4", "1.2.3.4.5", "1.2.65536"):
    ...
    def test_stages_executable_frontend_assets_dlls_and_store_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(output.name, "HomeEnergyManager_1.66.3.0_x64.msix")
            ...
            self.assertEqual(identity.attrib["Version"], "1.66.3.0")
            ...
            bundle_dir = release / "bundle" / "msix"
            stage_path = bundle_dir / "stage"
            shutil.rmtree(stage_path)
            stage_path.write_text("stale file", encoding="utf-8")
            restaged, _ = prepare_msix.stage_msix(
                root,
                target,
                "0.66.3",
                "Store.Identity",
            ...
            self.assertEqual((restaged / "givenergy-local.exe").read_bytes(), b"exe")

            shutil.rmtree(restaged)
            old_stage = bundle_dir / "old-stage"
            old_stage.mkdir()
            stage_path.symlink_to(old_stage, target_is_directory=True)
            restaged, _ = prepare_msix.stage_msix(
                root,
                target,
                "0.66.3",
    ...
    def test_release_workflow_builds_an_unsigned_store_package(self) -> None:
        ...
        self.assertIn("MSIX_OUTPUT=", workflow)
        self.assertIn('Where-Object { $_ -like "MSIX_OUTPUT=*" }', workflow)
