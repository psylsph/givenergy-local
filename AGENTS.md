...
- **`forecast/`** — solar forecast foundation (issue #283, Phase 0). Fetches hourly Open-Meteo radiation + cloud cover on a 3 h tick that rides the weather loop, persists the forward window to `history.db::forecast_values` (upsert per timestamp+variable+source), and self-calibrates a site performance ratio (median of daily actual-vs-insolation ratios, rejecting dark / partial-poll / dead days; ≥ 5 usable days required) so the model never needs panel geometry. `SolarForecastProvider` trait leaves room for alternative sources.
