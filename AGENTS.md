...
- **Pages**: `StatusPage`, `BatteryPage`, `HistoryPage`, `ForecastPage` (issue #283 predictions), `ControlPage` (model-aware rate scaling, slot-labelling warnings), `InverterPage`, `PowerPage`, `SolarPage`, `MetersPage`, `SettingsPage`, `LogsPage` (developer mode only)
