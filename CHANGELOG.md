...
- **A corrupt meter read can no longer hijack the day's solar total.** On setups where the panels are measured by a CT clamp, a single bad read of the meter's energy counter could blow the day's solar figure up to nonsense (one reporter saw 1,128 kWh) and pin it there until midnight. Implausible jumps are now discarded and the counter re-seeds itself — the same corruption defence the rest of the energy figures already had. (fixes #294)
...
- **"Show Node Status Words" now defaults to on.** The status words are now the primary direction signal, so they're visible by default on the Status page. The toggle in Settings → Panel Controls is still there for anyone who prefers the bare value.
...
- **The Grid Voltage chart uses a tighter y-axis so fluctuations are readable.** Grid voltage sits in a narrow band (~230–240 V), so the old 0-based axis squashed the line flat against the top and hid the day-to-day variation. The axis now snaps to the recorded range ±10 V, giving the line room to breathe while keeping the absolute scale honest. See #152.
