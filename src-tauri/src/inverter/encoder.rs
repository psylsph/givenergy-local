...
pub enum ControlCommand {
    ...
    AgileChargeSlot {
        ...
        start_hhmm: u16,
        /// Packed HHMM end time of the cheap run.
        end_hhmm: u16,
        /// Target SOC for the slot (4-100).
        target_soc: u16,
    },
    /// Three-phase equivalent of `AgileChargeSlot`: writes HR 1113/1114
    /// (slot 1 times), HR 1111 (target SOC), HR 1123 (force-charge enable)
    /// and HR 1112 (AC-charge enable). Mirrors `ThreePhaseForceCharge`.
    ThreePhaseAgileChargeSlot {
        start_hhmm: u16,
        end_hhmm: u16,
        target_soc: u16,
    },
    /// Drive the inverter through its native slot schedule for an Agile
    /// expensive-window discharge (with export — `BATTERY_POWER_MODE = 0`
    /// for the duration of the slot). Single-phase variant (HR 56/57 +
    /// HR 59 + HR 27).
    AgileDischargeSlot {
        /// Packed HHMM start time of the expensive run.
        start_hhmm: u16,
        /// Packed HHMM end time of the expensive run.
        end_hhmm: u16,
    },
    ...
    AgileClearActiveSlot,
    /// Three-phase equivalent of `AgileClearActiveSlot`: writes HR 1123 = 0,
    /// HR 1112 = 0, HR 1122 = 0, HR 27 = 1, plus zeros HR 1113/1114 and
    ...
    pub fn encode(&self) -> Result<Vec<RegisterWrite>, String> {
        let writes = match self {
            ...
            ControlCommand::AgileDischargeSlot {
                ...
                // (option β — preserves today's force-discharge
                // export-revenue semantics). Outside the slot the
                // inverter obeys whatever schedule the user has armed.
                validate_hhmm(*start_hhmm, "agile discharge slot start")?;
                validate_hhmm(*end_hhmm, "agile discharge slot end")?;
                vec![
                    rw(HR_DISCHARGE_SLOT_1_START, *start_hhmm),
                    rw(HR_DISCHARGE_SLOT_1_END, *end_hhmm),
                    rw(HR_ENABLE_DISCHARGE, 1),
            ...
            ControlCommand::ThreePhaseAgileDischargeSlot {
                ...
                vec![
                    rw(HR_3PH_DISCHARGE_SLOT_1_START, *start_hhmm),
            ...
            ControlCommand::AgileClearActiveSlot => {
                // Idempotent disarm. Used on Agile scope=Off, on backend
                // restart (clears any slot the previous process had
                // preloaded), and on the mid-band transition out of a
                // charge or discharge slot. Leaves the user's manual
                // charge/discharge schedules untouched (the inverse
                // pattern from CosyExit, which always clears the discharge
                // flag — Agile mode is the only one that drives both
                // sides so it must clear both sides on exit).
                vec![
                    rw(HR_ENABLE_CHARGE, 0),
                    rw(HR_ENABLE_CHARGE_TARGET, 0),
                    rw(HR_ENABLE_DISCHARGE, 0),
                    rw(HR_CHARGE_SLOT_1_START, 0),
                    rw(HR_CHARGE_SLOT_1_END, 0),
                    rw(HR_DISCHARGE_SLOT_1_START, 0),
                    rw(HR_DISCHARGE_SLOT_1_END, 0),
                    rw(HR_BATTERY_POWER_MODE, 1), // restore eco
                ]
            }
            ControlCommand::ThreePhaseAgileClearActiveSlot => {
                vec![
                    rw(HR_3PH_FORCE_CHARGE_ENABLE, 0),
                    rw(HR_3PH_AC_CHARGE_ENABLE, 0),
                    rw(HR_3PH_FORCE_DISCHARGE_ENABLE, 0),
                    rw(HR_3PH_CHARGE_SLOT_1_START, 0),
                    rw(HR_3PH_CHARGE_SLOT_1_END, 0),
                    rw(HR_3PH_DISCHARGE_SLOT_1_START, 0),
                    rw(HR_3PH_DISCHARGE_SLOT_1_END, 0),
                    rw(HR_BATTERY_POWER_MODE, 1), // restore eco
                ]
            }
...
mod tests {
    ...
    // -----------------------------------------------------------------
    // Agile slot-based commands (slot-driven cheap/expensive windows)
    // -----------------------------------------------------------------
    ...
    #[test]
    fn three_phase_agile_charge_slot_writes_three_phase_slot_registers() {
        let cmd = ControlCommand::ThreePhaseAgileChargeSlot {
            start_hhmm: 1300,
            end_hhmm: 1330,
            target_soc: 100,
        };
        let writes = cmd.encode().unwrap();
        // Expect: power_mode + clear_discharge + slot_start + slot_end +
        // target_soc + ac_charge_enable + force_charge_enable = 7.
        assert_eq!(writes.len(), 7);
        assert_eq!(writes[0].address, HR_BATTERY_POWER_MODE);
        assert_eq!(writes[0].value, 1); // eco
        assert_eq!(writes[1].address, HR_3PH_FORCE_DISCHARGE_ENABLE);
        assert_eq!(writes[1].value, 0); // clear stale discharge
        assert_eq!(writes[2].address, HR_3PH_CHARGE_SLOT_1_START);
        assert_eq!(writes[2].value, 1300);
        assert_eq!(writes[3].address, HR_3PH_CHARGE_SLOT_1_END);
        assert_eq!(writes[3].value, 1330);
        assert_eq!(writes[4].address, HR_3PH_CHARGE_TARGET_SOC);
        assert_eq!(writes[4].value, 100);
        assert_eq!(writes[5].address, HR_3PH_AC_CHARGE_ENABLE);
        assert_eq!(writes[5].value, 1);
        assert_eq!(writes[6].address, HR_3PH_FORCE_CHARGE_ENABLE);
        assert_eq!(writes[6].value, 1);
    }

    #[test]
    fn agile_discharge_slot_writes_slot_times_and_export_mode() {
        // Drive a 16:00–18:00 expensive run. Option β semantics: flip
        // BATTERY_POWER_MODE = 0 (export) for the duration of the slot.
        let cmd = ControlCommand::AgileDischargeSlot {
            start_hhmm: 1600,
            end_hhmm: 1800,
        };
        let writes = cmd.encode().unwrap();
        ...
        assert_eq!(writes[1].value, 1800);
        assert_eq!(writes[2].address, HR_ENABLE_DISCHARGE);
        assert_eq!(writes[2].value, 1);
        assert_eq!(writes[3].address, HR_BATTERY_POWER_MODE);
        assert_eq!(writes[3].value, 0); // export mode for the slot
    }

    #[test]
    fn three_phase_agile_discharge_slot_writes_three_phase_registers() {
        let cmd = ControlCommand::ThreePhaseAgileDischargeSlot {
            start_hhmm: 1630,
            end_hhmm: 1700,
        };
        let writes = cmd.encode().unwrap();
        ...
        assert_eq!(writes[0].value, 1630);
        assert_eq!(writes[1].address, HR_3PH_DISCHARGE_SLOT_1_END);
        assert_eq!(writes[1].value, 1700);
        assert_eq!(writes[2].address, HR_3PH_FORCE_CHARGE_ENABLE);
        assert_eq!(writes[2].value, 0);
        assert_eq!(writes[3].address, HR_3PH_FORCE_DISCHARGE_ENABLE);
        assert_eq!(writes[3].value, 1);
        assert_eq!(writes[4].address, HR_BATTERY_POWER_MODE);
        assert_eq!(writes[4].value, 0); // export
    }

    #[test]
    fn agile_clear_active_slot_writes_disarm_and_restore_eco() {
        // Used on Agile scope=Off, on backend restart, and on mid-band
        // transitions out of a charge or discharge slot. Must clear
        // BOTH sides (charge + discharge) so a stale preloaded slot
        // can't continue to fire after the backend restarts.
        let cmd = ControlCommand::AgileClearActiveSlot;
        let writes = cmd.encode().unwrap();
        assert_eq!(writes.len(), 8);
        assert_eq!(writes[0].address, HR_ENABLE_CHARGE);
        assert_eq!(writes[0].value, 0);
        ...
        assert_eq!(writes[2].address, HR_ENABLE_DISCHARGE);
        assert_eq!(writes[2].value, 0);
        assert_eq!(writes[3].address, HR_CHARGE_SLOT_1_START);
        assert_eq!(writes[3].value, 0);
        assert_eq!(writes[4].address, HR_CHARGE_SLOT_1_END);
        assert_eq!(writes[4].value, 0);
        assert_eq!(writes[5].address, HR_DISCHARGE_SLOT_1_START);
        assert_eq!(writes[5].value, 0);
        assert_eq!(writes[6].address, HR_DISCHARGE_SLOT_1_END);
        assert_eq!(writes[6].value, 0);
        assert_eq!(writes[7].address, HR_BATTERY_POWER_MODE);
        assert_eq!(writes[7].value, 1); // restore eco
    }

    #[test]
    fn three_phase_agile_clear_active_slot_writes_three_phase_registers() {
        let cmd = ControlCommand::ThreePhaseAgileClearActiveSlot;
        let writes = cmd.encode().unwrap();
        assert_eq!(writes.len(), 8);
        assert_eq!(writes[0].address, HR_3PH_FORCE_CHARGE_ENABLE);
        assert_eq!(writes[0].value, 0);
        ...
        assert_eq!(writes[2].address, HR_3PH_FORCE_DISCHARGE_ENABLE);
        assert_eq!(writes[2].value, 0);
        assert_eq!(writes[3].address, HR_3PH_CHARGE_SLOT_1_START);
        assert_eq!(writes[3].value, 0);
        assert_eq!(writes[4].address, HR_3PH_CHARGE_SLOT_1_END);
        assert_eq!(writes[4].value, 0);
        assert_eq!(writes[5].address, HR_3PH_DISCHARGE_SLOT_1_START);
        assert_eq!(writes[5].value, 0);
        assert_eq!(writes[6].address, HR_3PH_DISCHARGE_SLOT_1_END);
        assert_eq!(writes[6].value, 0);
        assert_eq!(writes[7].address, HR_BATTERY_POWER_MODE);
        assert_eq!(writes[7].value, 1);
    }

    #[test]
    fn agile_charge_slot_rejects_invalid_hhmm() {
        // 2400 is the upper bound that the HHMM validator rejects.
        let cmd = ControlCommand::AgileChargeSlot {
            start_hhmm: 2400,
            end_hhmm: 430,
            target_soc: 100,
    ...
    #[test]
    fn agile_charge_slot_rejects_invalid_target_soc() {
        // Reference bounds: [4-100].
        let cmd = ControlCommand::AgileChargeSlot {
            start_hhmm: 200,
            end_hhmm: 430,
        ...
        let cmd2 = ControlCommand::AgileChargeSlot {
            start_hhmm: 200,
            end_hhmm: 430,
            target_soc: 200,
        };
        assert!(cmd2.encode().is_err());
    }

    #[test]
    fn cosy_exit_unchanged_after_agile_refactor() {
        // Regression guard for the "don't break Standard / Cosy mode"
        // promise. The CosyExit arm of encode() must produce the
        // byte-identical register write sequence as before the
        // slot-based Agile refactor. If this test starts failing after
        // a refactor, the Standard-mode Cosy path has been altered.
        let cmd = ControlCommand::CosyExit;
        let writes = cmd.encode().unwrap();
        let addresses: Vec<u16> = writes.iter().map(|w| w.address).collect();
        let values: Vec<u16> = writes.iter().map(|w| w.value).collect();
        assert_eq!(
            addresses,
            vec![
                HR_ENABLE_CHARGE,
            ...
            "CosyExit register sequence must not change"
        );
        assert_eq!(
            values,
            vec![0, 0, 0, 1],
            "CosyExit register values must not change"
        );
    }

    #[test]
    fn force_charge_unchanged_after_agile_refactor() {
        // Quick-Action Force Charge must produce byte-identical writes
        // to before the refactor. The slot-based Agile path is a
        // separate command and doesn't touch this one.
        let cmd = ControlCommand::ForceCharge { target_soc: 80 };
        let writes = cmd.encode().unwrap();
        let addresses: Vec<u16> = writes.iter().map(|w| w.address).collect();
        assert_eq!(
            ...
            "ForceCharge register sequence must not change"
        );
    }

    #[test]
    fn force_discharge_unchanged_after_agile_refactor() {
        // Quick-Action Force Discharge must produce byte-identical writes.
        // Note: today's ForceDischarge covers the entire day
        // (start=0, end=2359). The slot-based AgileDischargeSlot is a
        // new command that writes a specific window instead.
        let cmd = ControlCommand::ForceDischarge;
        let writes = cmd.encode().unwrap();
        let addresses: Vec<u16> = writes.iter().map(|w| w.address).collect();
        assert_eq!(
            ...
            "ForceDischarge register sequence must not change"
        );
        // Full-day discharge slot: today this is what ForceDischarge
        // writes so the inverter discharges all day long.
        assert_eq!(writes[4].value, 0); // start 00:00
        assert_eq!(writes[5].value, 2359); // end 23:59
    }
